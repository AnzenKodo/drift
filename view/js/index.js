...
<div class="actions">
    ...
    <button class="actions_react" onclick="open_emoji_piker(this)">
        React <span class="actions_react_count">0</span>
    </button>
    <emoji-picker style="display: none" class="dark">
    { "id": "${id}", "pubkey": "${pubkey}" }
    </emoji-picker>
</div>`;
...
function open_emoji_piker(ele) {
    const emoji = ele.nextElementSibling;
    const emoji_display = emoji.style.display;
    if (emoji_display === "") {
    ...
    emoji.addEventListener('emoji-click', (evt) => {
        const ele = evt.srcElement;
        const { id, pubkey } = JSON.parse(ele.textContent);
        const detail = evt.detail;
        const emoji = detail.unicode;
        send_reaction(emoji, id, pubkey);
    });
}
